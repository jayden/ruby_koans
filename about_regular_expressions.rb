...
class AboutRegularExpressions < Neo::Koan
  ...
  #
  # Why?
  # They are considered greedy because there are many matches/not specific patterns
  ...
  # because z is an optional pattern, it's "a" because the match is the first a
  ...
  # if the pattern is not at the start of the string, match is nil
  end

  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/]
    assert_equal nil, "start end"[/start\z/]
  # likewise for the end of the string
  end
  ...
  #
  # character class = single character out of several possible characters
  # alternation = single regular expression out of several possible regexes
  ...
  # the pattern in the sub method matches "two" and the block takes the "two"
  # and returns the specific range (in this case "t") and replaces "two" with "t"
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] }

  # gsub is similar to sub, except it will find&replace all matches - not just the
  # leftmost match
  end
end
